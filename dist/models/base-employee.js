...
exports.queryBaseEmp = void 0;
const _database_1 = require("./_database");
exports.queryBaseEmp = {
    ...
    get(id) {
        let _self = this;
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT base_employee.* FROM base_employee WHERE base_employee.id=?`, [id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    getPerson() {
        let _self = this;
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT * FROM base_person INNER JOIN base_employee ON base_person.id = base_employee.person_id`, (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    insert(data) {
        ...
        return new Promise((resolve, reject) => {
            _database_1.db.query(`INSERT INTO base_employee 
              (
                person_id,
                employee_no,
                date_created
                ...
                ?,?,
                NOW()
              )`, [data.person_id, data.employee_no], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    update(data) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`UPDATE base_employee 
          SET
            person_id=?, 
            employee_no=?,
            date_created=?
          WHERE
            id=?
          `, [data.person_id, data.employee_no, data.date_created, data.id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    delete(id) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`DELETE FROM base_employee WHERE id=?`, [id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    getNewEmployeeNo() {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT MAX(substring(employee_no, 5, 5)) + 1 as employee_no FROM base_employee `, (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    checkPersonIfEmployee(personId) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT * FROM base_employee WHERE person_id = ?`, [personId], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
