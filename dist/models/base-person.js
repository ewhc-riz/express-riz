...
exports.queryBasePerson = {
            ...
            _database_1.db.query(queryCount + whereClause, (err0, result) => {
                ...
                _database_1.db.query(query + whereClause, (err, results) => {
                    ...
                    results.map(function (row, index) {
                        ...
                        row["date_of_birth"] = (0, moment_1.default)(row.date_of_birth).format("YYYY-MM-DD");
                    });
                    return resolve({
                        list: results,
    ...
    insert(data) {
        ...
        return new Promise((resolve, reject) => {
          ...
          (
            ...
            ?,?,?,?,
            NOW()
          )`, [data.first_name, data.last_name, data.date_of_birth, data.gender], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    delete(id) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`DELETE FROM base_person WHERE id=?`, [id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
