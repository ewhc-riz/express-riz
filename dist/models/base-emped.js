...
exports.queryBaseEmpEd = void 0;
const _database_1 = require("./_database");
exports.queryBaseEmpEd = {
    ...
    getEducation(id) {
        let _self = this;
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT base_employee_education.* FROM base_employee_education WHERE base_employee_education.id=?`, [id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    insert(data) {
        ...
        return new Promise((resolve, reject) => {
            _database_1.db.query(`INSERT INTO base_employee_education 
          (
            employee_id, 
            level,
            school_name,
            ...
            ?,?,?,?,
            NOW()
          )`, [data.employee_id, data.level, data.school_name, data.year_graduated], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    update(data) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`UPDATE base_employee_education 
          SET
            level = ?,
            school_name = ?,
            year_graduated = ?
            ...
            WHERE
            id=?
          `, [data.level, data.school_name, data.year_graduated, data.id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    delete(id) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`DELETE FROM base_employee_education WHERE id=?`, [id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    checkEducationIfExists(employee_id, level) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT * FROM base_employee_education WHERE employee_id = ? AND level = ?`, [employee_id, level], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
