...
exports.queryBaseUser = void 0;
const _database_1 = require("./_database");
exports.queryBaseUser = {
    ...
    get(id) {
        let _self = this;
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT base_user.* FROM base_user WHERE base_user.id=?`, [id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    getPerson() {
        let _self = this;
        return new Promise((resolve, reject) => {
            _database_1.db.query(`SELECT id, person_id FROM base_user ORDER by person_id DESC`, (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    insert(data) {
        ...
        return new Promise((resolve, reject) => {
            _database_1.db.query(`INSERT INTO base_user 
              (
                person_id,
                username,
                password,
                ...
                ?,?,?,
                NOW()
              )`, [data.person_id, data.username, data.password], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    update(data) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`UPDATE base_user 
          SET
            person_id=?, 
            username=?,
            password=?,
          ...
          WHERE
            id=?
          `, [data.person_id, data.username, data.password, data.date_created, data.id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
    ...
    delete(id) {
        return new Promise((resolve, reject) => {
            _database_1.db.query(`DELETE FROM base_user WHERE id=?`, [id], (err, results) => {
                if (err) {
                    console.log("Reject error:", err);
                    return reject(err);
                }
